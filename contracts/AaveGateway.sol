...
contract AaveGateway is ERC20Burnable, Ownable, MathUtils {
    ...
    // IERC20(0x9a693Dc6a317631e757e185575699D95C4707c73);

    IERC20 public constant DAIToken =
        IERC20(0x9a693Dc6a317631e757e185575699D95C4707c73);

    // IERC20(0x4F96Fe3b7A6Cf9725f59d353F723c1bDb64CA6Aa);
    // IERC20 public constant A_DAI =
    ...
    AggregatorV3Interface internal constant PRICE_FEED =
        AggregatorV3Interface(0x9326BFA02ADD2366b30bacB125260Af641031331);
    IUniswapRouter public constant UNISWAP_ROUTER =
    ...
    function borrow(address _address, uint256 _amount) external {
        require(_amount <= _borrowLimit(), "No collateral enough");
        usersBorrowed[_address] += _amount;
    ...
    function approveDAi(uint256 _amount, address spender) external {
        DAIToken.approve(spender, _amount);
    }
    ...
    function harvestRewards() external onlyOwner {
        uint256 aWethBalance = A_WETH.balanceOf(address(this));
    ...
    function _sendWethToAave(uint256 _amount) internal {
        WETH_GATEWAY.depositETH{value: _amount}(
            address(AAVE),
            address(this),
            0
        );
    ...
    function _withdrawWethFromAave(uint256 _amount) internal {
        A_WETH.approve(address(WETH_GATEWAY), _amount);
        WETH_GATEWAY.withdrawETH(address(AAVE), _amount, address(this));
    }

    function getCollateral(address _address) external view returns (uint256) {
        return usersCollateral[_address];
    }

    function getBorrowed(address _address) external view returns (uint256) {
        return usersBorrowed[_address];
    }

    // function balance() public view returns (uint256) {
    //     return A_DAI.balanceOf(address(this));
    // }
    ...
    function _convertEthToDai(uint256 _amount) internal returns (uint256) {
        ...
        uint256 amountOut = UNISWAP_ROUTER.exactInputSingle{value: _amount}(
            params
        );
